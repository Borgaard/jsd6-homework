...
Stock.prototype.buyShares = function(number) {
      ...
      this.shares += number;
      console.log("This is how many shares you have " +this.shares);
}

Stock.prototype.sellShares = function(number) {
    if ((this.shares - number) < 0) { 
        console.log("Error");
      } else {
      this.shares -= number;
      console.log( "thanks for selling now you have "+this.shares);
      } 
}
Stock.prototype.calcValue = function(price) {
    var amount = (this.shares * price);
    var roundedAmount = amount.toFixed(2);
 console.log("the total value of your shares is " +roundedAmount)
}
...
console.log(apple.exchange);
console.log(visa.exchange);
visa.sellShares(240);
